...
const hero = {
  ...
  legenda: "Curso de Produtos de Limpeza",
}

const destaques = [
  { src: "/limpeza-destaque-1.mp4", legenda: "Produção de detergentes" },
  { src: "/limpeza-destaque-2.mp4", legenda: "Técnicas especiais" },
]

const videos = [
  { src: "/limpeza-video-01.mp4", legenda: "Preparação dos ingredientes" },
  { src: "/limpeza-video-05.mp4", legenda: "Mistura e fragrâncias" },
  { src: "/limpeza-video-09.mp4", legenda: "Embalagem e rotulagem" },
  { src: "/limpeza-video-13.mp4", legenda: "Resultado final" },
]

const todos = [hero, ...destaques, ...videos]
...
function PlayOverlay() {
  return (
    <div className="absolute inset-0 flex items-center justify-center bg-black/[0.06] group-hover/card:bg-black/15 transition-colors duration-500 rounded-xl">
      <div className="w-9 h-9 rounded-full bg-white/85 backdrop-blur-sm shadow-lg flex items-center justify-center transition-transform duration-300 group-hover/card:scale-110">
        <Play className="h-4 w-4 text-rose-500 ml-0.5" />
      </div>
    </div>
  )
}

function CardHero({ src, legenda, onClick }: { src: string; legenda: string; onClick: () => void }) {
...
function CardVertical({ src, legenda, index, onClick }: {
  src: string; legenda: string; index: number; onClick: () => void
}) {
  const [ref, show] = useFadeIn(index * 50)
  return (
    <div ref={ref}
...
function CardSquare({ src, legenda, index, onClick }: {
  ...
  const [ref, show] = useFadeIn(index * 40)
  return (
    <div ref={ref}
      className="group/card cursor-pointer transition-all duration-600 ease-out"
...
export default function CursoGaleriaLimpeza() {
  ...
  return (
    <section className="mb-14">
      ...
      <div className="max-w-5xl mx-auto px-3 sm:px-6 space-y-4 sm:space-y-5">
        <CardHero src={hero.src} legenda={hero.legenda} onClick={() => open(0)} />

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          <div className="sm:col-span-1 sm:row-span-2">
            <CardVertical src={destaques[0].src} legenda={destaques[0].legenda} index={1} onClick={() => open(1)} />
          </div>

          <div className="sm:col-span-2 grid grid-cols-2 gap-3 sm:gap-4 content-start">
            {videos.map((item, i) => (
          ...
          <div className="sm:col-span-1 sm:row-span-2">
            <CardVertical src={destaques[1].src} legenda={destaques[1].legenda} index={2} onClick={() => open(2)} />
          </div>
        </div>
      </div>
